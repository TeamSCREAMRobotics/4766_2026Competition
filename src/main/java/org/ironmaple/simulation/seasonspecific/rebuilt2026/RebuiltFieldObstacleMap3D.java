...
public class RebuiltFieldObstacleMap3D extends SimulatedArena3D.FieldMap3D {
  ...
  public RebuiltFieldObstacleMap3D() {
    ...
    this.addBorderLine(
        new Translation3d(
            FIELD_WIDTH + wallThickness / 2, FIELD_HEIGHT / 2, allianceWallHeight / 2),
        new Translation3d(wallThickness, FIELD_HEIGHT, allianceWallHeight));
    ...
    addObstacle(
        "meshes/hub.obj", new Pose3d(blueHubPos.getX(), blueHubPos.getY(), 0, new Rotation3d()));

    // Red Hub
    Translation2d redHubPos = RebuiltHub.RED_HUB_POS.toTranslation2d();
    addObstacle(
        "meshes/hub.obj",
        new Pose3d(redHubPos.getX(), redHubPos.getY(), 0, new Rotation3d(0, 0, Math.PI)));
    ...
    addObstacle(
        "meshes/bump.obj",
        new Pose3d(4.614, FIELD_HEIGHT - 2.505, 0.0, new Rotation3d(0, 0, Math.PI)));
    ...
    addObstacle(
        ...
        new Pose3d(trenchX, trenchHingedBaseY, 0, new Rotation3d()));
    addObstacle(
    ...
    addObstacle(
        "meshes/trench_fixed.obj",
        new Pose3d(FIELD_WIDTH - trenchX, trenchFixedY, 0, new Rotation3d()));
