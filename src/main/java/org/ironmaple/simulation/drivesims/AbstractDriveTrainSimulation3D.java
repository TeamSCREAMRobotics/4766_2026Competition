 ...
 */
public abstract class AbstractDriveTrainSimulation3D extends Body
    implements SimulatedArena3D.Simulatable {
  public static final double
      BUMPER_COEFFICIENT_OF_FRICTION =
          0.02, // https://en.wikipedia.org/wiki/Friction#Coefficient_of_friction
   ...
   */
  protected AbstractDriveTrainSimulation3D(
  ...
  public void setRobotSpeeds(ChassisSpeeds givenSpeeds) {
    ...
    double vx =
        givenSpeeds.vxMetersPerSecond * Math.cos(heading)
            - givenSpeeds.vyMetersPerSecond * Math.sin(heading);
    double vy =
        givenSpeeds.vxMetersPerSecond * Math.sin(heading)
            + givenSpeeds.vyMetersPerSecond * Math.cos(heading);

    physicsBody.setLinearVelocityMPS(new Translation3d(vx, vy, 0));
    physicsBody.setAngularVelocityRadPerSec(
        new Translation3d(0, 0, givenSpeeds.omegaRadiansPerSecond));
  }
   ...
   *
   * <p>For AdvantageScope and similar tools where robot models have their origin at the bottom
   * (wheel contact point), this method returns a pose with Z adjusted so Z=0 represents the bottom
   * of the robot at rest on flat ground.
   *
  ...
  public Pose2d getSimulatedDriveTrainPose() {
    ...
    return new Pose2d(pose3d.getX(), pose3d.getY(), new Rotation2d(pose3d.getRotation().getZ()));
    // System.out.println(GeometryConvertor.toWpilibPose2d(transform));
