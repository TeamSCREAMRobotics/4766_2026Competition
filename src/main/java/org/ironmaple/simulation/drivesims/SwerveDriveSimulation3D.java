...
public class SwerveDriveSimulation3D extends AbstractDriveTrainSimulation3D {
  ...
  private static final double MAX_SUSPENSION_FORCE =
      1000.0; // 4 wheels * 1000N = 4000N = 400kg support
  ...
  public void simulationSubTick(int subTickNum) {
    ...
    if (isThreaded && !calculatorRegistered && proxy != null) {
      double tickPeriodSeconds =
          arena.getPhysicsTickPeriodSeconds()
              / org.ironmaple.simulation.SimulatedArena3D.getPhysicsSubTicksPerFrame();
      physicsCalculator =
          new org.ironmaple.simulation.physics.threading.SwervePhysicsCalculator(
    ...
    if (subTickNum == 0) {
      ...
      SimDebugLogger.logHeading(
          String.format(
              "Heading2d=%.1f° (from toRotation2d)", Math.toDegrees(heading2d.getRadians())));
    }
    ...
    double dYaw =
        edu.wpi.first.math.MathUtil.angleModulus(
            Rotation2d.fromRadians(currentBodyYaw)
  ...
  /**
   * Calculates the critical damping coefficient for a given mass per wheel. c_crit = 2 * sqrt(k *
   * m)
   */
   ...
   */
  private Translation3d rotatePoint(
      Translation3d point, edu.wpi.first.math.geometry.Rotation3d rotation) {
  ...
  public Pose3d getSimulatedDriveTrainPose3dGroundRelative() {
    ...
    Translation2d rotatedXYOffset = comXYOffset.rotateBy(physicsPose.getRotation().toRotation2d());
   ...
   */
  private void simulateModulesThreaded(
      Pose3d pose3d, Translation3d linearVel, ThreadedPhysicsProxy proxy) {
    var rotation = pose3d.getRotation();
    ...
    for (int i = 0; i < moduleSimulations.length; i++) {
      ...
      module.updateSimulationSubTickGetModuleForce(
          groundVelocity2d, robotHeading, normalForcePerWheel);
  ...
  private void simulateModulesSync(Pose3d pose3d, Translation3d linearVel, int bodyId) {
    ...
    for (int i = 0; i < moduleTranslations.length; i++) {
      ...
      Translation3d localMountPoint =
          new Translation3d(moduleOffset.getX(), moduleOffset.getY(), mountPointZ);
      ...
      double maxRayDistance =
          fixedRayStartHeight + suspensionRestLength + SUSPENSION_MAX_TRAVEL + wheelRadius;
      ...
      org.dyn4j.geometry.Vector2 tractionForce2d =
          module.updateSimulationSubTickGetModuleForce(
              groundVelocity2d, robotHeading, normalForceNewtons);
