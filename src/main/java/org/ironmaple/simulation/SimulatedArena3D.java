...
public abstract class SimulatedArena3D implements Arena {
  ...
  public static synchronized void resetInstance() {
    ...
    if (instance == null) {
      ...
      instance = new Arena2026Rebuilt3D(defaultPhysicsConfig);
    }
  }
  ...
  public NetworkTable redTable =
      NetworkTableInstance.getDefault()
          .getTable("SmartDashboard/MapleSim3D/MatchData/Breakdown/Red Alliance");
  public NetworkTable blueTable =
      NetworkTableInstance.getDefault()
          .getTable("SmartDashboard/MapleSim3D/MatchData/Breakdown/Blue Alliance");
   ...
   *
   * <p>When running threaded, this is the actual physics time step. When sync, this is
   * 1/tickRateHz.
   *
  ...
  /**
   * Whether to wait for physics frame completion in threaded mode. When true, simulationPeriodic()
   * blocks until physics is done, ensuring the freshest state is available but reducing
   * parallelism.
   *
  ...
  protected final Set<AbstractDriveTrainSimulation3D> driveTrainSimulations;
  ...
  protected SimulatedArena3D(FieldMap3D fieldMap, PhysicsThreadConfig threadConfig) {
    ...
    if (instance != null) {
      throw new IllegalStateException(
          "SimulatedArena3D already created! Only one arena instance is allowed. "
    ...
    if (RobotBase.isReal() && (!ALLOW_CREATION_ON_REAL_ROBOT)) {
      throw new IllegalStateException(
          "MapleSim3D is running on a real robot! "
    ...
    final Vector2 EARTH_GRAVITY = new Vector2(0.0, -9.81);
    this.physicsWorld.setGravity(EARTH_GRAVITY);
    this.physicsBackend = new org.ironmaple.simulation.physics.jolt.JoltBackend(config);
    this.physicsBackend.initialize();
    this.physicsEngine =
        ((org.ironmaple.simulation.physics.jolt.JoltBackend) physicsBackend).getEngine();
    ...
    for (FieldMap3D.Obstacle obstacle : fieldMap.getObstacles()) {
      ...
      if (shape != null) {
        ...
        System.out.println(
            "[MapleSim3D] Created static body: "
                + (obstacle.meshResourcePath() != null
                    ? obstacle.meshResourcePath()
                    : "Primitives"));
        System.out.println("[MapleSim3D] Global Pose: " + obstacle.pose());
      } else {
        System.err.println(
            "[MapleSim3D] Failed to create shape for obstacle: "
                + (obstacle.meshResourcePath() != null
                    ? obstacle.meshResourcePath()
                    : "Primitives"));
      }
   ...
   */
  public static void overrideInstance(SimulatedArena3D newInstance) {
    if (instance != null) instance = new Arena2026Rebuilt3D();
   ...
   *
   * <p>In synchronous mode (default), runs multiple sub-ticks per period. In threaded mode with
   ...
   * </ol>
   *
   * <p>This lock-step approach ensures determinism: physics only runs when explicitly signaled,
  ...
  public synchronized void simulationPeriodic() {
    synchronized (SimulatedArena3D.class) {
      ...
      SmartDashboard.putBoolean(
          "MapleSim3D/LockStepMode", physicsBackend.isThreaded() && isLockStepMode());
   ...
   *
   * <p>Lock-step mode ensures deterministic physics by requiring explicit frame signals from the
   * main thread, eliminating wall-clock timing jitter.
   *
   ...
   *
   * <p>In lock-step mode (default), physics only runs when the main thread signals a frame,
   * ensuring deterministic behavior. In free-running mode, physics runs at its configured tick rate
   ...
   *
   * <p>When true (default), simulationPeriodic() blocks until physics completes, ensuring the
   * freshest state is available. When false, physics runs in parallel (pipelined), using the
   ...
   *
   * <p>In threaded mode, returns the duration of the last background physics tick (pure physics
   * time).
   *
   * <p>In sync mode, returns the duration of the last simulationPeriodic call (physics + custom
   * sims).
   *
  ...
  public void spawnGamePieceOnField(
    ...
    GamePieceOnFieldSimulation3D.GamePieceInfo3D info3d =
        new GamePieceOnFieldSimulation3D.GamePieceInfo3D(
            info.type(), shape, info.gamePieceMass(), 0.8, 0.1, 0.1, 0.5);

    GamePieceOnFieldSimulation3D originalPiece =
        new GamePieceOnFieldSimulation3D(this, info3d, pose);
  ...
  /**
   * Registers a game piece with the arena's dynamic bodies tracking. This ensures it can be removed
   * properly later.
   *
  ...
  public void registerGamePieceVisualizationSource(
      String type, java.util.function.Consumer<List<Pose3d>> source) {
  ...
  protected final Map<String, List<java.util.function.Consumer<List<Pose3d>>>>
      visualizationSources = new HashMap<>();
   ...
   *
   * @see #getGamePiecesPosesByType(String)
   * @deprecated Use {@link #getGamePiecePosesArrayByType(String)} instead. This method name was
   *     confusing as it implied returning GamePiece objects.
   */
  @Deprecated
  ...
  public static class FieldMap3D {
    ...
    public static class Obstacle {
      ...
      public Obstacle(
          PhysicsShape shape, Pose3d pose, Translation3d halfExtents, String meshResourcePath) {
        this.shape = shape;
        this.pose = pose;
    ...
    protected void addRectangularObstacle(
        double lengthX, double widthY, double heightZ, Pose3d pose) {
     ...
     */
    protected void addBorderLine(
        Translation2d start, Translation2d end, double height, double thickness) {
